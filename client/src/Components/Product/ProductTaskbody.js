...
class TaskBody extends Component {
  ...
  createtask = e => {
    ...
    tasknames.push(e);
    this.setState({ createflag: 0, alltask: alltask, tasknames: tasknames });
    this.setState(state => ({ tasknumber: -1 }));
  };
  createoldtask = () => {
  ...
  render() {
    ...
    if (this.state.subtask === -1) {
      ...
      for (var i = 0; i < this.props.tasklist.length; i++) {
        ...
        if (i === this.state.tasknumber && this.state.createflag === 1) {
          tasklist.push(
            <CreateTask
              createnexttask={this.createnexttask}
      ...
      task = (
        <ProductSubtaskbody
          ...
          task_name={this.props.tasklist[this.state.subtask].task_name}
          subtasklist={this.props.tasklist[this.state.subtask].subtask}
